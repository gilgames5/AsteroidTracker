    ...
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private async Task RefreshAsteroidData()
        {
            try
            {
                ...
                {
                    System.Windows.MessageBox.Show("No asteroid data found for the selected date range.", "No Data");
                    return;
                }
                ...
                foreach (var dateEntry in nearEarthObjects.Children<JProperty>())
                {
                    ...
                    {
                        var closeApproachData = asteroid["close_approach_data"];
                        if (closeApproachData == null || !closeApproachData.Any())
                        {
                            continue;
                        }

                        var closeApproach = closeApproachData[0];
                        ...
                        Asteroids.Add(new AsteroidModel
                        {
                            ...
                            CloseApproachDate = DateTime.Parse((string)closeApproach["close_approach_date"]),
                            DistanceKm = double.Parse((string)closeApproach["miss_distance"]["kilometers"]),
                            SpeedKps = double.Parse((string)closeApproach["relative_velocity"]["kilometers_per_second"]),
                            ...
                            IsHazardous = (bool?)asteroid["is_potentially_hazardous_asteroid"] ?? false,
